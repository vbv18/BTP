"""
Multi-SKU Demand Forecasting System - Phase 1 (Industry-Style)
GLOBAL MODEL with Weekly Aggregation for Sparse Data
Uncertainty-aware forecasting with P10, P50, P90 quantiles
FINAL VERSION - Improved reproducibility, evaluation, and interpretability
"""
...
FORECAST_WEEKS = 26  # 26 weeks ≈ 6 months
INTERMITTENT_THRESHOLD = 0.6  # 60% zeros for intermittent classification
...
def aggregate_to_weekly(df):
    """Convert daily demand to weekly aggregation"""
...
def classify_skus(df):
    """Classify SKUs as smooth or intermittent"""
    classification = {}
    zero_ratios = {}
...
def calculate_sku_stats(df):
    """Calculate per-SKU statistics for global model features"""
    sku_stats = {}
...
def create_features(df, sku_stats):
    """
    Feature engineering for global model
    IMPROVED: Returns SKU mapping for consistent encoding
    """
    df = df.copy()
    ...
    # NEW: Create and store SKU mapping for reproducible encoding
    ...
    # NEW: Return SKU mapping along with data
...
def train_global_model(train_df, quantile):
    """
    Train global LightGBM model with sample weights
    NEW: Returns feature importance for P50 model
    """
    features = [
...
def forecast_intermittent_sku(sku_data, forecast_weeks):
    """
    Probabilistic forecasting for intermittent SKUs
    IMPROVED: SKU-specific random seed for reproducibility
    """
...
def forecast_all_skus(df, classification, sku_stats, sku_mapping, forecast_weeks):
    ...
    IMPROVED: Uses stable SKU mapping for encoding
    """
    ...
    # NEW: Print feature importance for P50 model
...
def generate_naive_baseline(df, forecast_weeks):
    """
    NEW: Generate naive baseline forecast (last observed value)
    Used for comparison to demonstrate model improvement
    """
    baseline_forecasts = []
...
def evaluate_forecasts(forecast_df, actual_df, baseline_df=None):
    """
    Evaluate forecasts with time-based train/test split and baseline comparison
    IMPROVED: Time-based evaluation and baseline comparison
    """
    # Convert actual data to weekly
    ...
    # NEW: Time-based split - evaluate on last 20% of available data
...
# ============================================================================
# MAIN EXECUTION (MODIFIED SECTION)
# ============================================================================
def main():
    ...
    # ========================================================================
    # SAVE OUTPUTS - CSV AND EXCEL
    # ========================================================================
    print(f"Saving forecasts to {OUTPUT_FILE}...")
    forecasts.to_csv(OUTPUT_FILE, index=False)
    
    # NEW: Excel export alongside CSV
    # If error occurs, install: pip install openpyxl
    try:
